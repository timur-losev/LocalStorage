...
namespace {
    ...
    StringData getNextToken(const StringData& currentString,
        ...
        for (; index < currentString.size(); index++) {
            if (terminalChars.find(currentString[(unsigned)index]) != std::string::npos) {
                break;
            }
        }
    ...
    Status parseTm(const StringData& dateString,
        ...
        // colon
        if (minEnd != std::string::npos && dateString[(unsigned)minEnd] == ':') {
            // Make sure the string doesn't end with ":"
            if (minEnd == dateString.size() - 1) {
                StringBuilder sb;
                sb << "Invalid date:  " << dateString << ".  Ends with \"" << dateString[(unsigned)minEnd]
                   << "\" character";
                return Status(BadValue, sb.str());
            }
        ...
        // is a period
        if (secEnd != std::string::npos && dateString[(unsigned)secEnd] == '.') {
            // Make sure the string doesn't end with "."
            if (secEnd == dateString.size() - 1) {
                StringBuilder sb;
                sb << "Invalid date:  " << dateString << ".  Ends with \"" << dateString[(unsigned)secEnd]
                   << "\" character";
                return Status(BadValue, sb.str());
            }
        ...
        else if (secEnd != std::string::npos && dateString[(unsigned)secEnd] != '.') {
            tzStr = getNextToken(dateString, "", secEnd, &tzEnd);
        }
        else if (minEnd != std::string::npos && dateString[(unsigned)minEnd] != ':') {
