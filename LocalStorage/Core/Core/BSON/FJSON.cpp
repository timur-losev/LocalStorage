...
#include "FErrorcodes.h"
#include "FStatus.h"
#include "FBase64.h"
#include "FBSONobjbuilder.h"
#include "FHex.h"
...
//        : _buf(str), _input(str), _input_end(str + strlen(str)) {}
    ...
    Status JParse::value(const StringData& fieldName, FBSONObjBuilder& builder)
    {
    ...
    Status JParse::object(const StringData& fieldName, FBSONObjBuilder& builder, bool subObject) {
        MONGO_JSON_DEBUG("fieldName: " << fieldName);
        if (!readToken(LBRACE)) {
        ...
        if (peekToken(RBRACE)) {
            readToken(RBRACE);
            if (subObject) {
                FBSONObjBuilder empty(builder.subobjStart(fieldName));
                empty.obj();
            }
            return Status::OK();
        }
        ...
        if(!reserved) { // firstField != <reserved field name>
            ...
            FBSONObjBuilder* objBuilder = &builder;
            unique_ptr<FBSONObjBuilder> subObjBuilder;
            if (subObject) {
                subObjBuilder.reset(new FBSONObjBuilder(builder.subobjStart(fieldName)));
    ...
    Status JParse::objectIdObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        std::string id;
    ...
    Status JParse::binaryObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        ...
        builder.appendBinData( fieldName, (int)binData.length(),
                BinDataType(fromHex(binDataType)),
                binData.data());
        return Status::OK();
    }

    Status JParse::dateObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        errno = 0;
    ...
    Status JParse::timestampObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        if (!readToken(LBRACE)) {
    ...
    Status JParse::regexObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        std::string pat;
    ...
    Status JParse::dbRefObject(const StringData& fieldName, FBSONObjBuilder& builder) {

        FBSONObjBuilder subBuilder(builder.subobjStart(fieldName));
    ...
    Status JParse::undefinedObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
        if (!readToken("true")) {
    ...
    Status JParse::numberLongObject(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(COLON)) {
            return parseError("Expected ':'");
        }
    ...
    Status JParse::array(const StringData& fieldName, FBSONObjBuilder& builder) {
        MONGO_JSON_DEBUG("fieldName: " << fieldName);
        uint32_t index(0);
        if (!readToken(LBRACKET)) {
            return parseError("Expected '['");
        }
        FBSONObjBuilder subBuilder(builder.subarrayStart(fieldName));
    ...
    Status JParse::constructor(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (readToken("Date")) {
    ...
    Status JParse::date(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(LPAREN)) {
            return parseError("Expected '('");
        }
        errno = 0;
    ...
    Status JParse::timestamp(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(LPAREN)) {
            return parseError("Expected '('");
        }
        if (readToken("-")) {
    ...
    Status JParse::objectId(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(LPAREN)) {
            return parseError("Expected '('");
        }
        std::string id;
    ...
    Status JParse::numberLong(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(LPAREN)) {
            return parseError("Expected '('");
        }
        errno = 0;
    ...
    Status JParse::numberInt(const StringData& fieldName, FBSONObjBuilder& builder) {
        if (!readToken(LPAREN)) {
            return parseError("Expected '('");
        }
        errno = 0;
        ...
        int32_t val = (int32_t)strtol(s.c_str(), NULL, 10);
        if (errno == ERANGE) {
    ...
    Status JParse::dbRef(const StringData& fieldName, FBSONObjBuilder& builder) {
        FBSONObjBuilder subBuilder(builder.subobjStart(fieldName));
    ...
    Status JParse::regex(const StringData& fieldName, FBSONObjBuilder& builder) {
    ...
    Status JParse::number(const StringData& fieldName, FBSONObjBuilder& builder) {
        double retd;
    ...
    _AttrAlwaysInline bool JParse::peekToken(const char* token) {
        assert(*token);
        assert(token[1] == 0);
        return peek() == *token;
    }

    _AttrAlwaysInline bool JParse::readToken(const char* token) {
    ...
    _AttrAlwaysInline bool JParse::match(char matchChar, const char* matchSet) const {
        if (matchSet == NULL) {
            return true;
        }
        if (*matchSet == '\0') {
    ...
    FBSONObj fromjson(std::istream& i, FBSONObjBuilder& builder)
    {
        if (i.eof())
        {
            return FBSONObj();
    ...
    /*
    FBSONObj fromjson(const std::string& str, FBSONObjBuilder& b) {
