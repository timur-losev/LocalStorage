...
#pragma GCC diagnostic ignored "-Wstring-compare"
    ...
    _AttrAlwaysInline unsigned endian(unsigned x) {
        return big ? _byteswap_ulong(x) : x;
    }
    _AttrAlwaysInline short endian_short(short x) {
        return big ? _byteswap_ushort(x) : x;
    }
    _AttrAlwaysInline long long endian_ll(long long x) {
    ...
    _AttrAlwaysInline int endian_int(int x) {
        return endian(x);
    }
    _AttrAlwaysInline double endian_d(double x) {
        long long z = endian_ll((long long&)x);
        return (double&)z;
    }
    _AttrAlwaysInline int readInt(const char *p) {
