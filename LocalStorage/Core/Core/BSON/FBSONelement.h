...
namespace _bson {
    ...
    int compareElementValues(const FBSONElement& l, const FBSONElement& r);


    /** FBSONElement represents an "element" in a FBSONObj.  So for the object { a : 3, b : "abc" },
        'a : 3' is the first element (key+value).

        The FBSONElement object points into the FBSONObj's data.  Thus the FBSONObj must stay in scope
        for the life of the FBSONElement.
    ...
    */
    ...
    public:
        /** Used when we already know the field name length, to avoid
            ...
            */
        explicit FBSONElement(int fnSize, const char *d) :
        ...
        */
        ...
        FArray<FBSONElement> Array() const; // see implementation for detailed comments
        ...
        /** @return the embedded object associated with this field.
            ...
            returned FBSONObj if you need your own copy.
        ...
        */
        FBSONElement operator[] (const std::string& field) const;
        ...
        */
        ...
        FBSONObj wrap( const StringData& newName) const;
        ...
        */
        ...
        FBSONObj object() const;

        /* uasserts if not an object */
        FBSONObj embeddedObjectUserCheck() const;

        FBSONObj codeWScopeObject() const;
        ...
        /** like operator== but doesn't check the fieldname,
            just the value.
        */
        bool valuesEqual(const FBSONElement& r) const
        {
            return woCompare( r , false ) == 0;
        }

        /** Returns true if elements are equal. */
        bool operator==(const FBSONElement& r) const
        {
            return woCompare( r , true ) == 0;
        }
        /** Returns true if elements are unequal. */
        bool operator!=(const FBSONElement& r) const { return !operator==(r); }
        ...
        */
        int woCompare( const FBSONElement &e, bool considerFieldName = true ) const;
        ...
        bool operator<( const FBSONElement& other ) const 
        {
        ...
        explicit FBSONElement(const char *d, int maxLen) : data(d)
        {
            if ( eoo() )
            {
                totalSize = 1;
        ...
        explicit FBSONElement(const char *d) : data(d) 
        {
            fieldNameSize_ = -1;
            totalSize = -1;
            if ( eoo() ) {
        ...
        /** Construct a FBSONElement where you already know the length of the name. The value
         ...
         *  size.
         */
        FBSONElement(const char* d, int fieldNameSize, FieldNameSizeTag)
            : data(d)
    ...
    private:
        ...
        friend class FBSONObj;

        const FBSONElement& chk(int t) const
        {
            if ( t != type() ) {
                StringBuilder ss;
                if( eoo() )
        ...
        const FBSONElement& chk(bool expr) const
        {
    ...
    _AttrAlwaysInline bool FBSONElement::trueValue() const {
    ...
    _AttrAlwaysInline bool FBSONElement::isNumber() const {
        switch( type() ) {
        case NumberLong:
        case NumberDouble:
        case NumberInt:
    ...
    _AttrAlwaysInline bool FBSONElement::isSimpleType() const {
        switch( type() ) {
        case NumberLong:
        case NumberDouble:
        case NumberInt:
    ...
    _AttrAlwaysInline double FBSONElement::numberDouble() const {
        switch( type() ) {
        case NumberDouble:
    ...
    _AttrAlwaysInline int FBSONElement::numberInt() const {
        switch( type() ) {
        case NumberDouble:
    ...
    /** Retrieve long value for the element safely.  Zero returned if not a number. */
    _AttrAlwaysInline long long FBSONElement::numberLong() const {
        switch( type() ) {
        case NumberDouble:
      ...
      _AttrAlwaysInline FBSONElement::FBSONElement() {
        static char z = 0;
        data = &z;
        fieldNameSize_ = 0;
        totalSize = 1;
