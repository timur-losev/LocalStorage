...
namespace _bson {
    ...
    /** iterator for a FBSONObj

       Note each FBSONObj ends with an EOO element: so you will get more() on an empty
       object, although next().eoo() will be true.

       The FBSONObj must stay in scope for the duration of the iterator's execution.
    ...
    */
    ...
    public:
        ...
        */
        FBSONObjIterator(const FBSONObj& jso) {
            int sz = jso.objsize();
            if ( sz == 0 ) {
                _pos = _theend = 0;
                return;
        ...
        FBSONObjIterator( const char * start , const char * end ) {
            _pos = start + 4;
            _theend = end - 1;
        }
        ...
        FBSONElement next( bool checkEnd ) {
            verify( _pos <= _theend );
            ...
            FBSONElement e( _pos, maxLen );
            int esize = e.size( maxLen );
            massert( 16446, "FBSONElement has bad size", esize > 0 );
            _pos += esize;
        ...
        FBSONElement operator*() {
            verify( _pos <= _theend );
            return FBSONElement(_pos);
        }
...
#define BSONForEach(e, obj)                                       \
    ::mongo::FBSONObjIterator BOOST_PP_CAT(it_,__LINE__)(obj);     \
