...
namespace _bson {
    ...
    /**
     * Create a FBSONObj from a JSON <http://www.json.org>,
     ...
     */
     FBSONObj fromjson(std::istream&, FBSONObjBuilder& builder);

    /** @param len will be size of JSON object in text chars. */
     //FBSONObj  fromjson(const char* str, int* len=NULL);

    /**
     * Parser class.  A FBSONObj is constructed incrementally by passing a
     * FBSONObjBuilder to the recursive parsing methods.  The grammar for the
    ...
    public:
        explicit JParse(std::istream&);
             ...
             */
        private:
            _bson::Status value(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
        public:
            _bson::Status object(const StringData& fieldName, FBSONObjBuilder&, bool subObj=true);
        ...
        private:
             ...
             */
            _bson::Status objectIdObject(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status binaryObject(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * DATEOBJECT :
             *     { FIELD("$date") : <64 bit signed integer for milliseconds since epoch> }
             */
            _bson::Status dateObject(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status timestampObject(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status regexObject(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status dbRefObject(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * UNDEFINEDOBJECT :
             *     { FIELD("$undefined") : true }
             */
            _bson::Status undefinedObject(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * NUMBERLONGOBJECT :
             *     { FIELD("$numberLong") : "<number>" }
             */
            _bson::Status numberLongObject(const StringData& fieldName, FBSONObjBuilder&);
             ...
             *   | VALUE , ELEMENTS
             */
            _bson::Status array(const StringData& fieldName, FBSONObjBuilder&);
             ...
             *     DATE
             */
            _bson::Status constructor(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status date(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status timestamp(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * OBJECTID :
             *     ObjectId( <24 character hex string> )
             */
            _bson::Status objectId(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * NUMBERLONG :
             *     NumberLong( <number> )
             */
            _bson::Status numberLong(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * NUMBERINT :
             *     NumberInt( <number> )
             */
            _bson::Status numberInt(const StringData& fieldName, FBSONObjBuilder&);

            /*
             * DBREF :
             *     Dbref( <namespace string> , <24 character hex string> )
             */
            _bson::Status dbRef(const StringData& fieldName, FBSONObjBuilder&);
             ...
             *     g | i | m | s
             */
            _bson::Status regex(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _bson::Status number(const StringData& fieldName, FBSONObjBuilder&);
             ...
             */
            _AttrAlwaysInline bool peekToken(const char* token);
             ...
             */
            _AttrAlwaysInline bool readToken(const char* token);
        ...
        public:
            _AttrAlwaysInline long long offset() { return _offset; }
