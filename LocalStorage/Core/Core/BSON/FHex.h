...
namespace _bson {
    ...
    _AttrAlwaysInline int fromHex( char c ) {
        if ( '0' <= c && c <= '9' )
            return c - '0';
        if ( 'a' <= c && c <= 'f' )
            return c - 'a' + 10;
    ...
    _AttrAlwaysInline char fromHex( const char *c ) {
        return (char)(( fromHex( c[ 0 ] ) << 4 ) | fromHex( c[ 1 ] ));
    }
    _AttrAlwaysInline char fromHex( const StringData& c ) {
        return (char)(( fromHex( c[ 0 ] ) << 4 ) | fromHex( c[ 1 ] ));
    }

    _AttrAlwaysInline std::string toHex(const void* inRaw, int len) {
    ...
    _AttrAlwaysInline std::string toHexLower(const void* inRaw, int len) {
        static const char hexchars[] = "0123456789abcdef";
    ...
    _AttrAlwaysInline std::string integerToHex(int i) {
