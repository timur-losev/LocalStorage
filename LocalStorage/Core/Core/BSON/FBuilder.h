...
///        void decouple() { data = 0; }
        ...
        {
            FMemory::memcpy(grow((int) len), src, len);
        }
        ...
        _AttrAlwaysInline char* grow(int by)
        {
            int oldlen = l;
            int newLen = l + by;
            if ( newLen > size ) {
    ...
    typedef _BufBuilder<FMemory> BufBuilder;
    ...
    public:
        ...
        void write( const char* buf, int len) { FMemory::memcpy( _buf.grow( len ) , buf , len ); }
    ...
    typedef StringBuilderImpl<FMemory> StringBuilder;
